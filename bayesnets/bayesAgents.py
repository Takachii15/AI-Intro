...
def constructBayesNet(gameState):
    ...
    Construct an empty Bayes net according to the structure given in the
    project description.
    ...
    - a large number of "observation" variables for each cell Pacman can
      measure
    ...
    for var in obsVars:
        for housVar in HOUSE_VARS:
            edges.append((housVar, var))
...
def fillObsCPT(bayesNet, gameState):
    """
    This funcion fills the CPT that gives the probability of an observation in
    each square, given the locations of the food and ghost houses.
    ...
    autograder, this function uses the *food house distribution* over colors
    when both the food house and ghost house are assigned to the same cell.
    """
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            obsVar = OBS_VAR_TEMPLATE % obsPos
            newObsFactor = bn.Factor([obsVar],
                                     [GHOST_HOUSE_VAR, FOOD_HOUSE_VAR],
                                     bayesNet.variableDomainsDict())
    ...
    def goToBest(self, gameState):
        ...
        tx, ty = dict(
            zip([
                ...
                bestFoodAssignment[FOOD_HOUSE_VAR]]
        bestAction = None
        bestDist = float("inf")
